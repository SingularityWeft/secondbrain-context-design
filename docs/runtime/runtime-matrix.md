...
`Unterstützt` ist kein pauschaler Vertraulichkeitsclaim. Der primäre technische Pfad beweist nur die lokale Prozessgrenze für den angegebenen synthetischen Stand; KI-Inferenz und externe Anbieterkommunikation wurden absichtlich nicht ausgeführt. Für einen privaten Praxistest entscheidet der User bewusst über Agent, Anbieter, Datenklasse und minimale Aufgabensicht.

Für nicht einzeln getestete Agents gilt: Das Markdown-/JSON-Format kann manuell bereitgestellt werden, aber native Dateirechte, Datenverarbeitung, Modellqualität und Isolation sind `unverified`. Ein Agentenname allein öffnet kein Datengate; die Nutzung bewusst ausgewählter echter Informationen ist eine ausdrückliche Entscheidung des jeweiligen Users, keine Sicherheitsfreigabe durch dieses Repository.
